...
//		File f3 = new File("D:\\tmp\\test.txt");
		...
		try {
			f1.createNewFile(); //createNewFile메소드가 실행할 ㅒㄸ 실제 파일이 만들어짐
			f2.createNewFile();
			...
			System.out.println(tmpFolder.isFile()); //false
			...
			folder.mkdir();
			 ...
			 System.out.println("파일경로 : " + file.getAbsolutePath());//C:\workspace\1_JAVA\javastudy\parent\person.txt
			 System.out.println("파일용량 : " + file.length());//0
			 System.out.println("파일상위폴더 : " + file.getParent());//parent
