...
public class Run {
	 ...
	 * 
	 * 오버라이딩 전 : Object 클래스의 toString()실행 -> 풀글래스명 + @ + 객체의 주소를 암호화한 16진수 값
	 * 오버라이팅 후 : Man클래스(자식클래스)의 toString()를 실행 -> 해당 객체가 가지고 있는 모든 필드의 값을 한 문자열로 반환
	 * 
	 * *오버라이딩
	 * - 자식클래스가 상속받고 있는 부모 클래스의 메소드를 제정의 하는 것
	 * - 부모가 제공하는 메소드를 자식이 일부 고쳐서 사용하겠다는 의미
	 * 		자식객체를 통해 실행시 자식 메소드가 우선권을 가지고 있다.
	 * 
	 * *오버라이딩 성립조건
	 * - 부모메소드명과 메소드명이 동일
	 * - 매개변수의 갯수, 자료형, 순서 동일(매개변수명 상관x)
	 * - 반환형이 동일
