...
//		System.out.println(dArr.hashCode());
		 ...
		 * 
		 * 기본자료형(원시타입) : boolean, int, char, byte, long, double 등등으로 선언된 변수
		 * => 실제 리터럴값을 바로 담을 수 있다.
		 * 
...
//		System.out.println(iArr[3]);
		...
		double[] dArr = new double[4]; //0~3 번 인텍스 사용가능, 0.0으로 초기화 되어있음
//		System.out.println(dArr[1]);
		for(int i=0; i < dArr.length; i++) {
			System.out.println(dArr[i]);
		}
		
		//내가 각 인덱스의 초기화를 하지 않아도 값들이 담겨잇다.
		//Heap이라는 메모리공간은 절대 빈공간을 허용하지 않는다.
		//=> 공간이 만들어질 때 JVM이 기본값으로라도 초기화를 진행한다.
		
		System.out.println(iArr); //배열의 자료형 + @ + 주소값의 16진수 형태 //[I@2f4d3709
		System.out.println(iArr.hashCode()); // 주소값의 10진수 형태 // 793589513
		 ...
		 *   -> 항상 오류가 발생한다.
		 *  
		 *  System.out.println(arr.hashCode()); //arr는 null 이라서 오류가 뜸
		 */
